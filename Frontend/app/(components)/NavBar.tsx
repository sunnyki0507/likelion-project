...
export default function NavBar({ changeViewAction, tagFilterState, currentView }: NavBarProps) {
  const pathname = usePathname()
  const [isFilterModalOpen, setIsFilterModalOpen] = useState(false)
      ...
      <header className="border-b border-gray-200 w-full">
        <div className="w-full px-4 py-4 flex justify-between items-center max-w-screen-xl mx-auto">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            <button
              onClick={() => changeViewAction("Favorites")}
              className={`cursor-pointer font-medium ${currentView === "Favorites" ? "text-black" : "text-gray-600 hover:text-black"}`}
            >
              Favorites
            </button>
            ...
            <button onClick={handleChangeView} className="font-medium text-gray-600 hover:text-black transition-colors">
              Change view
            </button>

            <button
              onClick={() => changeViewAction("Profile")}
              className={`font-medium ${currentView === "Profile" ? "text-black" : "text-gray-600 hover:text-black"}`}
            >
              Profile
            </button>
          </nav>
