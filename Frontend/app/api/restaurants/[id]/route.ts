...
export async function GET(
  ...
  console.log('Found restaurant info:', info ? 'yes' : 'no');
  if (!info) {
...
function getInfo(id: string) {
	// Get a consistent index for any ID by hashing it
	const hash = id.split('').reduce((acc, char) => {
		return char.charCodeAt(0) + ((acc << 5) - acc);
	}, 0);
	const index = (Math.abs(hash) % 6) + 1; // This will give us a number between 1 and 6
	
	// Get the mock data for this index
	const mockData = data[index.toString()];
	if (!mockData) return undefined;

	// Return the mock data with the restaurant's own ID
	return {
		...mockData,
